...
if !(exists('g:sh_ignore_no_md5') && g:sh_ignore_no_md5)
  let s:name = $VIMRUNTIME.'/syntax/sh.vim'
  let s:md5sum = split(system('md5sum '.shellescape(s:name)))[0]
  if !v:shell_error
   if  s:md5sum != "8f25facdc5b80c46b8b06f42a04dbc99" &&
      \s:md5sum != "2437480baab953fc393882587990f0fe" &&
      \s:md5sum != "c5d07d4c66208eadfedb24caa0117628" &&
      \s:md5sum != "cb71b369d3ab9d792c67226724f55d4c" &&
      \s:md5sum != "871782bb0c65d356e969099e16acc1c0"
...
" trying to answer the question: which shell is /bin/sh, really? {{{1
...
if !exists("s:sh_fold_subshell")
 let s:sh_fold_subshell = and(g:sh_fold_enabled,8)
endif
if !exists("s:sh_fold_linecont")
 let s:sh_fold_linecont = and(g:sh_fold_enabled,16)
endif
...
if s:sh_fold_subshell
 com! -nargs=* ShFoldSubShell <args> fold
else
 com! -nargs=* ShFoldSubShell <args>
endif
if s:sh_fold_linecont
 "TODO: extend line cont for implied continuations + \<CR>
 com! -nargs=* ShFoldLineCont <args> fold
else
 com! -nargs=* ShFoldLineCont <args>
endif
...
" Subshells: {{{1
" ==========
ShFoldSubShell  syn region shExpr  transparent matchgroup=shExprRegion  start="{" end="}"		contains=@shExprList2 nextgroup=shSpecialNxt
ShFoldSubShell  syn region shSubSh transparent matchgroup=shSubShRegion start="[^(]\zs(" end=")"	contains=@shSubShList nextgroup=shSpecialNxt
...
if exists("b:is_kornshell") || exists("b:is_bash") || exists("b:is_posix")
 ShFoldSubShell syn region shCommandSub matchgroup=shCmdSubRegion start="\$("  skip='\\\\\|\\.' end=")"  contains=@shCommandSubList
 ShFoldSubShell syn region shArithmetic matchgroup=shArithRegion  start="\$((" skip='\\\\\|\\.' end="))" contains=@shArithList
 ShFoldSubShell syn region shArithmetic matchgroup=shArithRegion  start="\$\[" skip='\\\\\|\\.' end="\]" contains=@shArithList
 syn match  shSkipInitWS contained	"^\s\+"
elseif !exists("g:sh_no_error")
 ShFoldSubShell syn region shCommandSub matchgroup=Error start="\$(" end=")" contains=@shCommandSubList
endif
ShFoldSubShell syn region shCmdParenRegion matchgroup=shCmdSubRegion start="(\ze[^(]" skip='\\\\\|\\.' end=")" contains=@shCommandSubList
...
ShFoldLineCont syn region  shSingleQuote	matchgroup=shQuote start=+'+ end=+'+		contains=@Spell	nextgroup=shSpecialStart,shSpecialSQ
ShFoldLineCont syn region  shDoubleQuote	matchgroup=shQuote start=+\%(\%(\\\\\)*\\\)\@<!"+ skip=+\\"+ end=+"+	contains=@shDblQuoteList,shStringSpecial,@Spell	nextgroup=shSpecialStart
ShFoldLineCont syn region  shDoubleQuote	matchgroup=shQuote start=+"+ skip=+\\"+ end=+"+		contained contains=@shDblQuoteList,shStringSpecial,@Spell	nextgroup=shSpecialStart
