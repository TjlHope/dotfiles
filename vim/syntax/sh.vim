...
" AFAICT "." should be considered part of the iskeyword. {{{1
" Using iskeywords in syntax is dicey, so the following permits the user to
...
syn cluster shExprList2	contains=@shExprList1,@shCaseList,shTest,shFunctionKey,shFunctionOne,shFunctionTwo
...
" Overriden by me as $(..) is valid posix sh.
...
ShFoldLineCont syn region  shSingleQuote	matchgroup=shQuote start=+'+ end=+'+			contains=@Spell
ShFoldLineCont syn region  shDoubleQuote	matchgroup=shQuote start=+\%(\%(\\\\\)*\\\)\@<!"+ skip=+\\"+ end=+"+	contains=@shDblQuoteList,shStringSpecial,@Spell
ShFoldLineCont syn region  shDoubleQuote	matchgroup=shQuote start=+"+ skip=+\\"+ end=+"+			contains=@shDblQuoteList,shStringSpecial,@Spell
